...
main(){ # called recursively for upgrade without reboot
 ...
 case $(lsb_release -sc) in
  $olddistro)
   ...
   apt install $(iigrep 'apt|dpkg|gzip|e2fsprogs' | newv $distro)
  ...
  $distro)
   apt install $(for i in $(comm -23 <(grep '^ii ' $flist | egrep -v 'kf5kdelibs|libldb|libsmb|linux-gnu' | awk '{print $2}' | awk -F: '{print $1}' |
   ...
   re='bzip2|ptitu|exfat|exim|magick|2-ma|me-(br|sh|so)|cp-co|on-th|d2-pl|ev-cr|or-0-pl|ls-bi|kit-1|td-pk|io-ut|n3-(ke|ren|sm|rf|uri|web)|md-co|ib-me|apache|lynx|perl-tk|sy-rs|pcscd|realmd|rtmp|wayland|foomatic|paps|pinentry|gstream.*(ugly|libav|qt5)| (tk[0-9.]+|tix) '
